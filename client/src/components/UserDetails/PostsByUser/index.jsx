...
import { getPostsCreated } from '../../../actions/posts'
...
const PostsByUser = ({ user }) => {
	const { createdPosts: posts, createdNumberOfPages: numberOfPages, isFetchingCreatedPosts: isLoading } = useSelector((state) => state.posts) // Fetch UserData
	const [page, setPage] = useState(1)
	const dispatch = useDispatch()

	useEffect(() => {
		dispatch(getPostsCreated(user.result._id || user.result.googleId, page))
	}, [page])
