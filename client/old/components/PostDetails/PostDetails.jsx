			...
			<Paper className={`${classes.loadingPaperLight} ${mode === 'light' ? classes.loadingPaperLight : classes.loadingPaperDark}`} elevation={6}>
				<div className={classes.card}>
					<div className={classes.section}>
						...
						<Typography variant="h6" className={`${mode === 'light' ? classes.textColor : classes.darkTextColor}`}>Created by: {post.creator.name}</Typography>
						<Typography variant="body1" className={`${mode === 'light' ? classes.textColor : classes.darkTextColor}`}>{moment(post.createdAt).format('Do MMMM YYYY, dddd, h:mm A')}</Typography>
